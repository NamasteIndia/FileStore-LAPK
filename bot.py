...
class Bot(Client):
    ...
    async def start(self):
        ...
        if FORCE_SUB_CHANNEL1:
            try:
                ...
                self.LOGGER(__name__).info("\nBot Stopped. https://t.me/requestcenterr for support")
                sys.exit()
        ...
        if FORCE_SUB_CHANNEL4:
            try:
                ...
                self.LOGGER(__name__).info("\nBot Stopped. https://t.merequestcenterr")
                sys.exit()
        try:
            ...
            self.LOGGER(__name__).info("\nBot Stopped. Join https://t.me/requestcenterr")
            sys.exit()

        self.set_parse_mode(ParseMode.HTML)
        self.LOGGER(__name__).info(f"Bot Running..!\n\nCreated by \nhttps://t.me/requestcenterr")
        self.LOGGER(__name__).info(f"""Bot is UP!""")

        self.set_parse_mode(ParseMode.HTML)
        self.username = usr_bot_me.username
        self.LOGGER(__name__).info(f"Bot Running..! Made by @requestcenterr")   
        ...
        try: await self.send_message(OWNER_ID, text = f"<b><blockquote>- Bᴏᴛ Rᴇsᴛᴀʀᴛᴇᴅ by @requestcenterr</blockquote></b>")
        except: pass
    ...
    def run(self):
        ...
        self.LOGGER(__name__).info("Bot is now running. Thanks to Madarchod Rohit for screwing this good repo up.")
        try:
            loop.run_forever()
